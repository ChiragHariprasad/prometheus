...
async def process_event(event: dict):
    ...
    if not org_id or not customer_id:
        logger.warning(f"Invalid event missing org/customer: {event.get('event_id')}")
        return

    async with async_session_factory() as session:
        twin_service = TwinService(session, redis_client, qdrant_client, kafka_client)
        event_service = EventService(session, redis_client, kafka_client)

        try:
            await event_service.process_event(org_id, event)
            await twin_service.update_twin_from_event(org_id, customer_id, event)
